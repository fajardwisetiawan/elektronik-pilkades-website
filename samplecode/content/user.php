...
if($datadev = mysqli_fetch_array($sqldev)) {
	...
	if(isset($_POST['b_GetUser'])){
		...
		$result=mysqli_query($GLOBALS["___mysqli_ston"], "SELECT count(*) as total from tb_user");		
		...
		for($i=1; $i<=$n; $i++){ 
			$content="";
			$sqlSetAll = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM tb_user LIMIT ".$offset.",".$limitPaging."");
			while($dataSetAll = mysqli_fetch_array($sqlSetAll)){
		...
		if ($pinSet == null){
			...
			$sqluserUp = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM tb_user where pin=".$pinSet);
		}
