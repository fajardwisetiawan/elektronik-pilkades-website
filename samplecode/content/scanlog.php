...
function CreateUserJSON(){
	...
	$footer = "]}";
	$content = "";
	$sqlscan = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM tb_scanlog");
	$i=0;
...
if(isset($_POST['b_delScanDB'])){
	$querydel	= mysqli_query($GLOBALS["___mysqli_ston"], "delete from tb_scanlog");
	if($querydel){
	...
	while($session){
		...
		if($content->IsSession != $session){
			...
			if (!$handle) { 				
				...
				$dirname = dirname("")."/JSOnDataScanLog.txt";
			} 
			fclose($handle); 
		}	
